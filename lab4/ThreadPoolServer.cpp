...
template<class T>
TPServer::ValueContainer<T>::ValueContainer(T& data, const uchar* pepper, uchar*(*encryption)(const char* pass, const uchar* salt)): value(data), count(0)
{
...
template<class T>
TPServer::ValueContainer<T>::ValueContainer(T& data, std::string hash, std::string pepper): value(data), hash(hash), salt(pepper), count(0) {}
